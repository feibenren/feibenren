...
发送信息的时候，就群发给这个局域网中的所有人，所有人收到信息，核对接收人地址，如果不是就直接抛弃，是的话就接收

这就一个问题，同一时间只能有一个节点发送信息
...
#### 建立拓扑图
交换机的工作原理也很简单，自己维护一张mac表，里面记录着连接自己的计算机的mac地址(mac地址是固定在网卡上的编号，全球唯一)

刚开始这个表是空的，交换机就通过广播的形式，来采集mac地址

交换机每隔一段时间就会更新一下mac表

通过这种方式建立起了一张`网络拓扑图`(如果没有这张图，连这个网络中有几台计算机都无法知晓)

#### 发送数据

a发送数据给b，交换机只需要查找`发送的数据中目的地址的mac地址`，然后将数据通过b连接交换机的接口发出即可，`不再需要广播了`
...
- 物理层的010101多少个是一组(帧)
- 一个帧里面，哪些部分是数据，哪些部分是MAC地址

最后有一种协议`Ethernet`称为了主流标准

通过路由器连接的网络，形成了一个`局域网`

# 第三层(网络层)

第二层的那个网络不能太大，因为需要广播，多了的话，光是跟新mac表，交换机都受不了
...
这个其实得靠`数据连接层`所建立起来的`结构图`,靠广播，通过制定相关协议，就可以知道自己和谁相连

...
> 使用请求方(浏览器)和回复方(服务器)表示缓存的两方

### 1、怎么样实现缓存？

这个主要是请求方的事情，接收到数据保存到本地，再次请求该数据就直接使用本地的数据，不用再次发送请求了

但是如果回复方更改了数据，发送方还得知道，所以这里需要一个机制


- 请求方应该有一种`过期机制`，来确定是否需要再次请求最新数据

- 回复方应该有一种`验证机制`来确定是否有必要`再次`回复数据


-------------

这里的请求方就是浏览器，回复方就是服务器


# 初级版本(http 1.0)

这个版本只有请求方的`过期机制`,没有回复方的`验证机制`
...
- 3、请求方收到该回复，将资源a和过期时间缓存起来

如果过了一段时间，如果再次请求这个资源a
